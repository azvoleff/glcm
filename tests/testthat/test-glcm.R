...
#              expected=getValues(expected_textures_3x3$mean),
#              tolerance=.000001)
#
# expect_equal(get_pkg_glcm_texture('mean_ENVI', c(5, 7), c(2, 3)),
#              expected=getValues(expected_textures_5x7$mean),
#              tolerance=.000001)
#
...
test_that("GLCM run on a matrix works correctly", {
    expect_equal(glcm_corr_matrix,
                 expected=raster::as.matrix(expected_textures_3x3$correlation),
                 tolerance=.000001)
})
